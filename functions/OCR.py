...
def SaveImage(event):
    ...
    cv.imwrite(filename, display, params)
...
def Recognize(iplimage):
    ...
    for index, text in enumerate(full_text.split('\n')):
        ...
        try:
            ...
            most_common_filter_pos = cv.getTrackbarPos('Filter', 'frame')
            # add last text
...
def getthresholdedimg(hsv):
    yellow = cv.inRange(hsv, np.array((20, 100, 100)), np.array((30, 255, 255)))
    blue = cv.inRange(hsv, np.array((100, 100, 100)), np.array((120, 255, 255)))
    both = cv.add(yellow, blue)
    return both
...
if __name__ == '__main__':
    ...
    # GUI
    cv.createTrackbar('Threshold', 'frame', int(thresh), 255, nothing)
    cv.createTrackbar('Erode', 'frame', int(erosion_iters), 4, nothing)
    cv.createTrackbar('Filter', 'frame', int(most_common_filter), 10, nothing)
    # menu image
    menu = cv.imread("menu.png")
    ...
    while True:
        ...
        if (height > 10) and (width > 10):
            ...
            channel = 1
            display = cv.cvtColor(display, cv.COLOR_BGR2GRAY)
            thresh = cv.getTrackbarPos('Threshold', 'frame')
            display = cv.threshold(display, thresh, 255, cv.THRESH_BINARY)[1]
            kernel = np.ones((5, 5), np.uint8)
            erosion_iters = cv.getTrackbarPos('Erode', 'frame')
            display = cv.erode(display, kernel, iterations=erosion_iters)
            # Show selection
            cv.imshow('selection', display)
